...
class AppThemeColorSchemeTest {
  ...
  fun dynamicMode_preservesEverySystemAccentPair() {
    ...
    val resolved = resolveClassevivaColorScheme(
      settings = AppSettings(accentMode = AccentMode.DYNAMIC, dynamicColorEnabled = true),
      isDark = false,
  ...
  fun fixedPalettes_tintContainerSurfacesAndKeepReadableContentPairs() {
    val brand = resolveClassevivaColorScheme(
      settings = AppSettings(accentMode = AccentMode.BRAND),
      isDark = false,
    )
    val orange = resolveClassevivaColorScheme(
      settings = AppSettings(accentMode = AccentMode.CUSTOM_PRESET, customAccentName = "ember"),
      isDark = false,
    )
  ...
  fun disabledDynamicMode_fallsBackToBrandInsteadOfUsingSystemRoles() {
    ...
    val resolved = resolveClassevivaColorScheme(
      settings = AppSettings(accentMode = AccentMode.DYNAMIC, dynamicColorEnabled = false),
      isDark = false,
      dynamicScheme = system,
    )

    assertEquals(classevivaBrandAccent(isDark = false), resolved.primary)
  ...
  fun amoledMode_keepsTheBackgroundTrulyBlack() {
    val resolved = resolveClassevivaColorScheme(
      settings = AppSettings(themeMode = ThemeMode.AMOLED, accentMode = AccentMode.BRAND),
      isDark = true,
    )
